# -*- coding: utf-8 -*-
"""
Full-Stack 11-Language Samanantar Translation & Qwen Chatbot Synchronizer
"""
import os
import json

# ==========================================
# 1. Update Backend indic_translation.py
# ==========================================
...
with open(r'C:\Users\aksha\.gemini\antigravity\scratch\scheme-sathi\backend\app\services\indic_translation.py', 'w', encoding='utf-8') as f:
    f.write(indic_code)
print("1. Updated backend/app/services/indic_translation.py")

# ==========================================
# 2. Update Backend chat.py
# ==========================================
    ...
    def query_qwen_ollama(
        ...
        for s in schemes_db[:15]:
            scheme_context_items.append(
                f"- Scheme: {s.scheme_name} ({s.code})\n"
                f"  Max Loan: Rs {s.max_loan_amount:,} | Subsidy: {s.subsidy_percentage}%\n"
                f"  Target: {s.target_category} | Eligible: {s.business_type} | Rural Subsidy: {s.rural_subsidy_percentage}%"
            )
...
"""
    ...
    @classmethod
    def _extract_matching_schemes(cls, message: str, schemes_db: List[Scheme]) -> List[Dict[str, Any]]:
        """Identify referenced schemes from user query to render interactive cards."""
        msg_lower = message.lower()
        matched = []
        for s in schemes_db:
            if s.code.lower() in msg_lower or any(word in msg_lower for word in s.scheme_name.lower().split() if len(word) > 3):
                matched.append({
                    "id": s.id,
                    "name": s.scheme_name,
                    "code": s.code,
                    "max_loan": s.max_loan_amount,
                    "subsidy": s.subsidy_percentage,
                    "tenor": s.max_repayment_period_months // 12
                })
                if len(matched) >= 3:
                    break
        return matched
    ...
    def _deterministic_fallback(
        ...
        # Check for greeting
        if any(w in msg_lower for w in ["hi", "hello", "namaste", "vanakkam", "namaskara", "nomoshkar", "kem cho", "sat sri akal"]):
            return {
        ...
        if any(w in msg_lower for w in ["vendor", "thela", "street", "10000", "50000", "patari"]):
            svanidhi = next((s for s in schemes_db if "SVANIDHI" in s.code), None)
            reply = f"{responses['found_schemes']}\n\n"
            if svanidhi:
                reply += f"• **{svanidhi.scheme_name} ({svanidhi.code})**\n  - Loan Limit: ₹10,000 - ₹50,000\n  - Collateral Free: 100%\n  - Interest Subsidy: 7% on timely repayment."
            return {
                "reply": reply,
                "source": "Rule Gazette Fallback",
                "language": lang,
        ...
        if any(w in msg_lower for w in ["woman", "women", "female", "mahila", "sc", "st"]):
            standup = next((s for s in schemes_db if "STANDUP" in s.code), None)
            mudra = next((s for s in schemes_db if "MUDRA" in s.code), None)
            reply = f"{responses['found_schemes']}\n\n"
            if standup:
                reply += f"• **{standup.scheme_name}** (₹10 Lakh - ₹1 Crore for SC/ST & Women)\n"
            if mudra:
                reply += f"• **{mudra.scheme_name}** (Up to ₹10 Lakh Collateral Free)\n"
            return {
                "reply": reply,
                "source": "Rule Gazette Fallback",
                "language": lang,
        ...
        pmegp = next((s for s in schemes_db if "PMEGP" in s.code), None)
        reply = f"{responses['found_schemes']}\n\n"
        if pmegp:
            reply += f"• **{pmegp.scheme_name} ({pmegp.code})**\n  - Up to ₹50 Lakh for Manufacturing\n  - Up to 35% Capital Subsidy for SC/ST/Women in Rural Areas."
        else:
...
with open(r'C:\Users\aksha\.gemini\antigravity\scratch\scheme-sathi\backend\app\services\chat.py', 'w', encoding='utf-8') as f:
    f.write(chat_code)
print("2. Updated backend/app/services/chat.py")

print("Backend modules updated successfully.")
